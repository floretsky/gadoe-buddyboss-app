...
const UpcomingEventsBlock = (props) => {
  const { block, wrapStyle, fontFamilyStyle, colors, config, accessToken } =
    props;
  ...
  const request = useMemo(() => {
    ...
    const url = isFullUrl
      ? query
        ? `${ds.route}${ds.route.includes('?') ? '&' : '?'}${query}`
        : ds.route
      : suffix;
  ...
  useEffect(() => {
    ...
    // HARD guards — if any of these fail, DO NOTHING
    if (!ds) return;
    if (!ds.route) return;
    if (!config) return;
    ...
    try {
      ...
      const routeSuffix = `wp-json${
        ds.route.startsWith('/') ? ds.route : `/${ds.route}`
      }`;

      api
        .customRequest(routeSuffix, 'get', {}, null, {}, false)
        .then((response) => {
          if (!mounted) return;
      ...
      // Catch *sync* crashes (this prevents SIGSEGV)
      console.log('UpcomingEventsBlock fatal error:', e);
      setErrorMsg('Unable to load events.');
      setLoading(false);
    }
  ...
  }, [block?.data?.data_source?.route, config]);
